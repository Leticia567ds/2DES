...
DELETE FROM Pedidos 
 ...
 select * from  Itens_Pedido where pizza_id = 18;
 select * from  Itens_Pedido where pizza_id = 22;

 select * from  Pizzas where pizza_id = 18;
...
select i.pedido_id, i.pizza_id,pz.nome,i.quantidade,i.valor
from Itens_pedido i
...
select p.pedido_id,p.cliente_id, c.nome as "Nome Clientes",p.data,p.hora, p.valor as "Total", py.nome, py.valor, i.valor*i.quantidade as "Subtotal"
from Itens_pedido i 
inner join pedidos p 
on p.pedido_id = i.pedido_id
inner join pizzas py
on  py.pizza_id =  i.pizza_id 
